...
class MLP {
   constructor(neuronsCounts, classes) {
      this.neuronsCounts = neuronsCounts;
      this.classes = classes;
      this.network = new NeuralNetwork(neuronsCounts);
   }
   load(mlp) {
      this.neuronsCounts = mlp.neuronsCounts;
   ...
   fit(samples, tries = 1000) {
      ...
      for (let i = 0; i < tries; i++) {
         this.network = new NeuralNetwork(this.neuronsCounts);
